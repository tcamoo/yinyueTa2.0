...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView }) => {
  const mainNavItems = [
    { id: View.HOME, label: '首映现场', sub: 'Premiere', icon: Home },
    { id: View.CHARTS, label: '流行音乐', sub: 'Popular Music', icon: Globe },
    { id: View.DJ, label: 'DJ 舞曲', sub: 'Club Zone', icon: Disc },
    { id: View.ARTICLES, label: '深度专栏', sub: 'Magazine', icon: FileText },
    { id: View.MV, label: 'MV 频道', sub: 'Visuals', icon: Film },
    { id: View.GALLERY, label: '视觉画廊', sub: 'Gallery', icon: Image },
    { id: View.PLAYLISTS, label: '我的歌单', sub: 'Playlists', icon: List },
  ];

  const adminNavItems = [
    { id: View.LIBRARY, label: '系统控制台', sub: 'Console', icon: LayoutGrid },
  ];

  return (
    <aside className="w-20 lg:w-72 h-screen flex flex-col justify-between fixed left-0 top-0 z-40 bg-transparent lg:pl-6 py-6 transition-all duration-300">
      ...
      <div className="w-full h-full glass rounded-[2rem] flex flex-col relative overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)]">
        ...
        {/* LOGO AREA - NEW AI SONIC WAVE DESIGN */}
        <div className="h-32 flex items-center justify-center lg:justify-start px-0 lg:px-8 relative z-10">
          <div className="relative group cursor-pointer" onClick={() => onChangeView(View.HOME)}>
             ...
             <div className="relative flex items-center gap-4">
               ...
               <div className="relative w-14 h-14 flex items-center justify-center bg-black/80 rounded-2xl border border-white/10 group-hover:border-brand-lime/50 transition-all duration-500 shadow-xl backdrop-blur-md overflow-hidden shrink-0">
                  ...
                  {/* SVG Logo: AI Sonic Wave / Abstract Y */}
                  <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg" className="relative z-10">
                      ...
                      {/* Bar 2 (Center) - Tallest, Main Beat */}
               ...
               <div className="hidden lg:flex flex-col">
                  ...
                  <div className="flex items-center justify-between w-full mt-1.5 pl-0.5">
                     ...
                     <div className="flex gap-0.5">
                        <span className="w-1 h-1 bg-brand-pink rounded-full animate-pulse"></span>
                        <span className="w-1 h-1 bg-brand-cyan rounded-full animate-pulse delay-75"></span>
                     </div>
                  </div>
               </div>
             </div>
        ...
        {/* NAVIGATION */}
        <nav className="flex-1 flex flex-col gap-3 px-3 lg:px-6 mt-4 overflow-y-auto scrollbar-hide">
          {mainNavItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`
                  group flex items-center justify-center lg:justify-start w-full p-4 rounded-xl transition-all duration-300 relative overflow-hidden
                  ${isActive 
              ...
              >
                ...
                <div className="hidden lg:flex flex-col ml-4 text-left relative z-10">
                   <span className={`font-bold text-base ${isActive ? 'text-white' : 'text-gray-300'}`}>
                     {item.label}
                   </span>
                   <span className={`text-[10px] uppercase font-display tracking-wider transition-colors ${isActive ? 'text-white/80' : 'text-gray-600 group-hover:text-gray-400'}`}>
                     {item.sub}
                   </span>
                </div>
                
                {!isActive && (
                    <div className="absolute inset-0 bg-brand-lime/10 translate-x-full group-hover:translate-x-0 transition-transform duration-300"></div>
                )}
              </button>
            );
          })}
             ...
             {adminNavItems.map((item) => {
                ...
                return (
                    <button
                        ...
                        onClick={() => onChangeView(item.id)}
                        className={`
                        group flex items-center justify-center lg:justify-start w-full p-4 rounded-xl transition-all duration-300 relative overflow-hidden mb-2
                        ${isActive 
                            ? 'bg-white/10 text-white border border-brand-lime/50 text-brand-lime shadow-[0_0_20px_rgba(255,255,255,0.1)]' 
                            : 'text-gray-500 hover:text-white hover:bg-white/5'}
                        `}
                    >
                        <Icon className={`w-5 h-5 relative z-10 ${isActive ? 'text-brand-lime' : 'group-hover:text-white'} transition-colors`} />
                        <div className="hidden lg:flex flex-col ml-4 text-left relative z-10">
        ...
        {/* VIP BADGE */}
        <div className="p-4 lg:p-6 pb-24 lg:pb-8 relative z-10">
          <div className="p-5 rounded-2xl bg-black border border-brand-lime/30 hidden lg:block relative overflow-hidden group cursor-pointer hover:border-brand-lime transition-all shadow-[0_5px_20px_rgba(0,0,0,0.5)]">
            ...
            <p className="text-xs text-gray-400 leading-relaxed font-mono">Hi-Res 无损 / 4K MV</p>
          </div>
        </div>
